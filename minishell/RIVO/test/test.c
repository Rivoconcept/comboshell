...
char	**format_argv(char *input)
{
    ...
    while (argv[i] != NULL)
    {
        if (i == 0  || tokens_type(argv[i]) == 1 || tokens_type(argv[i]) == 3
            || (tokens_type(argv[i]) == 2) || tokens_type(argv[i - 1]) == 1)
        {
...
void format_variable(char **argv, t_params *params)
{
    ...
    while(argv[i] != NULL)
    {
        tmp = ft_strdup(format_variable_env(argv[i], params));
        free(argv[i]);
        argv[i] = tmp;
        i++;
    }
