...
static e_tokentype token_type (char *rdir)
{
    if (!ft_strcmp(rdir, "<<"))
        return HERE_TOKEN;
...
s_element *parse_command(char *input)
{
    int i = 0;
    ...
    /*command = extract_word(input, &i);
    element_list = add_element(element_list, command);
    free(command);*/
    while (input[i]) {
        while (input[i] == ' ')
            i++;
        if (input[i] == '>' || input[i] == '<') {
            ...
            element_list = add_element(element_list, value, token_type(redir));
            free(redir);
            free(value);
        } else if (input[i]) {
            value = extract_word(input, &i);
